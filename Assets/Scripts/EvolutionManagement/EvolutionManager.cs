...
public class EvolutionManager : MonoBehaviour
{
    ...
    public void AddResultFromRound(GameResult result)
    {
        ...
        if (roundResults.Count == 0) 
        {
            ...
            for (int i = 0; i < this.numEvalRounds; i++) 
            {
                //ensure we can access by index.
                this.results[result.gameID].Add(null);
            }
        }
    ...
    {
        string g1Path = Constants.EVO_POPULATION_PATH + gid1;
        string g2Path = Constants.EVO_POPULATION_PATH + gid2;
        string gnewPath = Constants.EVO_POPULATION_PATH + newGameid;


        // Parent1 stuff
        SerializedPlayer parent1player1 = ReadJson<SerializedPlayer>(g1Path + Constants.PC_SLASH + Constants.PLAYER1_JSON);
        SerializedMove parent1player1move1 = ReadJson<SerializedMove>(g1Path + Constants.PC_SLASH + Constants.PLAYER1MOVE1_JSON);
        SerializedPlayer parent1player2 = ReadJson<SerializedPlayer>(g1Path + Constants.PC_SLASH + Constants.PLAYER2_JSON);
        SerializedMove parent1player2move1 = ReadJson<SerializedMove>(g1Path + Constants.PC_SLASH + Constants.PLAYER2MOVE1_JSON);
        Platforms parent1platforms = ReadJson<Platforms>(g1Path + Constants.PC_SLASH + Constants.LEVEL_JSON);

        // Parent2 stuff
        SerializedPlayer parent2player1 = ReadJson<SerializedPlayer>(g2Path + Constants.PC_SLASH + Constants.PLAYER1_JSON);
        SerializedMove parent2player1move1 = ReadJson<SerializedMove>(g2Path + Constants.PC_SLASH + Constants.PLAYER1MOVE1_JSON);
        SerializedPlayer parent2player2 = ReadJson<SerializedPlayer>(g2Path + Constants.PC_SLASH + Constants.PLAYER2_JSON);
        SerializedMove parent2player2move1 = ReadJson<SerializedMove>(g2Path + Constants.PC_SLASH + Constants.PLAYER2MOVE1_JSON);
        Platforms parent2platforms = ReadJson<Platforms>(g2Path + Constants.PC_SLASH + Constants.LEVEL_JSON);
        ...
        // Write out
        WriteJson<SerializedPlayer>(gnewPath + Constants.PC_SLASH + Constants.PLAYER1_JSON, newplayer1);
        WriteJson<SerializedMove>(gnewPath + Constants.PC_SLASH + Constants.PLAYER1MOVE1_JSON, newplayer1move1);
        WriteJson<SerializedPlayer>(gnewPath + Constants.PC_SLASH + Constants.PLAYER2_JSON, newplayer2);
        WriteJson<SerializedMove>(gnewPath + Constants.PC_SLASH + Constants.PLAYER2MOVE1_JSON, newplayer2move1);
        WriteJson<Platforms>(gnewPath + Constants.PC_SLASH + Constants.LEVEL_JSON, newPlatforms);
    }
    ...
    {
        if (!File.Exists(Constants.EVO_RESULTS_PATH))
        {
            Directory.CreateDirectory(Constants.EVO_RESULTS_PATH);
        }
        this.WriteJson<EvolutionResults>(Constants.EVO_RESULTS_PATH + Constants.PC_SLASH + Constants.EVO_RESULT_JSON, this.evolutionResults);
    }
